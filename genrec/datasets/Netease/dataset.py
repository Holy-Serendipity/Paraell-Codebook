...
class Netease(AbstractDataset):
    ...
    def _build_item_description(self, row: list) -> str:
        """从CSV行中提取关键字段，构建干净的文本描述。

        解析 topics (JSON数组) 和 fx_game_other_json (JSON对象)，
        提取 name、summary、gender、is_original_camera、allow_remake。
        """
        parts = []
                ...
                for row in tqdm(reader, total=total_lines, desc="speed", unit="row"):
                    if len(row) >= 3:
                        ...
                        if key not in item_ids:
                            continue
                        # 解析字段构建干净文本描述
                        value = self._build_item_description(row)

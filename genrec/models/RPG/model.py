...
class SwiGLUMLP(nn.Module):
    """SwiGLU Feed-Forward Network — 替换GPT-2的原生GELU MLP。

    SwiGLU(x) = down_proj(silu(gate_proj(x)) * up_proj(x))

    Reference: LLaMA, PaLM, HSTU
    """

    def __init__(self, n_embd: int, n_inner: int, dropout: float = 0.0):
        super().__init__()
        self.gate_proj = nn.Linear(n_embd, n_inner, bias=False)
        self.up_proj = nn.Linear(n_embd, n_inner, bias=False)
...
class RPG(AbstractModel):
    def __init__(
        ...
        # ── 可选：用SwiGLU替换GPT-2的GELU MLP ──
        self.use_swiglu = config.get('use_swiglu', False)
        if self.use_swiglu:
            self.tokenizer.log('[MODEL] Replacing GPT-2 GELU MLP with SwiGLU')
            for block in self.gpt2.h:
                block.mlp = SwiGLUMLP(
                    n_embd=config['n_embd'],
                    n_inner=config['n_inner'],
                    dropout=config.get('resid_pdrop', 0.0)
                )
    ...
    def _get_swing_batch_similarity(self, batch_items):
        ...
        if self._swing_sparse_matrix is None:
            try:
                ...
                print(f"[WARNING] Failed to init swing sim: {e}")
                return None
            if self._swing_sparse_matrix is None:
                return None
    ...
    def forward(self, batch: dict, return_loss=True) -> torch.Tensor:
        ...
        combine_states = outputs.last_hidden_state
        ...
        if return_loss:
            ...
            # 组间多样性损失
            group_similarity = torch.matmul(last_group_reps, last_group_reps.transpose(1,2))
